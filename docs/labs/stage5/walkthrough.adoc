...
During local testing you simulated messages sending them to _Kafka_, and bypassed pushing the CSV data to the Storage layer.

You will now deploy the process on OpenShift. Let's add the final S3 integration step and deploy.
...
+
Copy the snippet below into the process:
+
--
{copypaste}
----
        ...
        - to:
            uri: aws2-s3
            parameters:
              bucketNameOrArn: chatrooms-{user-username}
----
--
+
{empty} +
...
+
Open your `application.properties` and edit the following settings:
