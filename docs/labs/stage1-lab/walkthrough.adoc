...
In the next lab section you will inmediately start using Kaoto to start working on your Camel route definitions.
...
TIP: Camel JBang is an upstream tool for _Camel_. The tool is included in Red Hat build of Apache Camel under the name _Camel CLI_. It is an extremely useful tool for all things Camel. It simplifies many of the common tasks a Camel developer undergoes. 
...
. Open the first Camel route in the Kaoto editor. 
+
...
+
You should see the connection rendered as follows:
+
image::images/new/mapper-text.jpg[align="left", width=80%]
+
{blank}
+
The mapping above would simply copy the message from source to target, as in the _hello_ example below:
+
--
* `hello` -> `hello`
--
...
{blank}
+
However, we also want to include information about user and origin, like this:
+
--
...
--
+
Redefine the mapping rule following the steps below:
+
====
[loweralpha]
.... At the end of the row, click the [ {pen-btn} ] button to open the XPath editor (pop up window).
+
.... Delete all contents (current expression).
.... Copy and paste:
+
--
{copypaste}
